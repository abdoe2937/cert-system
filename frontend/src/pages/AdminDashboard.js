          ...
          <>
            <div className="p-5 space-y-3">
              ...
              {[
                ...
                { label: "اسم الكورس", key: "courseName" },
              ].map(({ label, key }) => (
                <div key={key}>
...
export default function AdminDashboard() {
  ...
  const tabBtn = (id, icon, label) => (
    <button
      ...
      className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium font-body transition-all ${activeTab === id ? "bg-gold-500/20 text-gold-300 border border-gold-500/30" : "text-slate-400 hover:text-slate-200"}`}
    >
      {icon} {label}
    </button>
  );
          ...
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-5 border-b border-slate-700/50">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                ...
                <button
                  onClick={fetchUsers}
                  className="btn-secondary py-2 px-3 flex items-center justify-center gap-1.5 text-sm"
                >
                  <RefreshIcon /> Refresh
                </button>
              </div>
