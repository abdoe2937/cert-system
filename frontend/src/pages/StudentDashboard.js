...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${s.cls}`}
    >
...
const ComplaintForm = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/complaints`, form, {
...
const ComplaintCard = ({ complaint }) => (
  <div className="card p-5 hover:border-slate-600/50 transition-colors">
    ...
    <p className="text-slate-400 text-sm font-body bg-slate-800/40 rounded-lg p-3 mb-3">
      {complaint.message}
    </p>
    {complaint.adminReply && (
      <div className="bg-gold-500/8 border border-gold-500/20 rounded-lg p-3">
        <p className="text-xs text-gold-400 font-medium font-body uppercase tracking-wider mb-1">
          Admin Reply
        </p>
        ...
        {complaint.repliedAt && (
          <p className="text-slate-500 text-xs mt-1 font-body">
            {new Date(complaint.repliedAt).toLocaleDateString("en-GB")}
          </p>
        )}
      </div>
    )}
...
export default function StudentDashboard() {
  ...
  const fetchComplaints = () => {
    ...
    const token = localStorage.getItem("token");
    axios
      .get(`${API_URL}/api/complaints/mine`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        >
          <div className="card p-5 border-gold-500/30 bg-gold-500/5">
            ...
            <p className="font-mono text-lg font-bold student-code">
              {user?.studentCode}
            </p>
          </div>
          <div className="card p-5">
            ...
            <p className="font-display text-xl font-bold text-white">
              {user?.isCompleted ? "✅ Completed" : "⏳ Active"}
            </p>
          </div>
          <div className="card p-5">
            <p className="text-xs text-slate-500 uppercase tracking-wider font-body mb-1">
        ...
        <div className="flex gap-1 mb-6 bg-slate-900/60 p-1 rounded-xl border border-slate-700/40">
          {tabs.map((tab) => (
            ...
            >
              ...
              {tab.count > 0 && (
                <span
                  className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? "bg-gold-500/30 text-gold-300" : "bg-slate-700 text-slate-400"}`}
                >
                  {tab.count}
                </span>
              )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                >
                  ...
                  <p className="text-slate-200 font-medium font-body">
                    {value || "—"}
                  </p>
                </div>
              ))}
            ...
            {loadingCerts ? (
              ...
              <div className="card p-12 text-center">
                ...
                <p className="text-slate-600 text-sm mt-1 font-body">
                  Complete your course to receive a certificate
                </p>
              </div>
            ) : (
              <div className="grid gap-4">
                {certificates.map((cert) => (
                  <div
                    key={cert._id}
                    className="card p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-gold-500/25 transition-colors"
                  >
        ...
        {activeTab === "complaints" && (
          <div className="animate-slide-up">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-slate-500 font-body bg-slate-800/60 px-2.5 py-1 rounded-full border border-slate-700/40">
...
{activeTab === 'enrollment' && (
  <div className="animate-slide-up">
    <EnrollmentPanel />
        ...
        {/* ── Suggested Courses — always visible below tabs ── */}
        <SuggestedCourses />
      </main>
    </div>
  );
